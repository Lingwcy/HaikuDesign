...
export const TypingEventsDemo = () => {
  ...
  const makeText = () =>
    `随机内容：${Math.random().toString(16).slice(2)}\n这行用于测试换行与空白符渲染。`
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-zinc-500">
          计数: {typingCount} / 完成: {completeCount}
        </span>
      </div>
...
export default function Demo() {
  ...
  <PlayBoard
    heading="吐字回调 + 自定义光标（typingCursor）"
    description="typing 仅对 string 生效；onTyping/onTypingComplete 可用于统计或联动 UI。"
